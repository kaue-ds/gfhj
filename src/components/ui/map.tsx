...
export function WorldMap({ 
  ...
  const projectPoint = (lat: number, lng: number) => {
    // These values are tuned to match the DottedMap projection
    const x = (lng + 180) * (800 / 360);
    // The y projection needs a slight offset to match the dotted map's internal projection
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-md bg-zinc-900 text-white border border-white/10 shadow-sm ${labelClassName}`}>
                          {dot.start.label}
                        </span>
                      </div>
                    </foreignObject>
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 bg-zinc-900 text-white px-3 py-2 rounded-lg text-sm font-medium backdrop-blur-sm sm:hidden border border-white/10"
          >
            {hoveredLocation}
          </motion.div>
        )}
